...
sw_tbin, sw_flbin, sw_flebin, _ = utils.lcbin(time=tim_sw, flux=fl_sw, binwidth=0.0005)
sw_tbin2, sw_flbin2, sw_flebin2, _ = utils.lcbin(time=tim_sw, flux=fl_sw, binwidth=0.005)
...
if f1.exists() and f2.exists():
    ...
    spk = spelunker.load(pid=1442, obs_num='1', visit='1', token='7ea4a28538834c32a798d837954d3f90', dir=pout, save=True)
...
## Binning them (for plotting purposes)
fg_tbin3, fg_flbin3, fg_flebin3, _ = utils.lcbin(time=tim_sw, flux=fbin, binwidth=0.0005)
fg_tbin4, fg_flbin4, fg_flebin4, _ = utils.lcbin(time=tim_sw, flux=fbin, binwidth=0.005)
...
for key in list(results_bin.keys()):
    ...
    ## For binning
    tbin5, res_bin5, rese_bin5, _ = utils.lcbin(time=tim_sw, flux=results_bin[key], binwidth=0.0005)
    tbin6, res_bin6, rese_bin6, _ = utils.lcbin(time=tim_sw, flux=results_bin[key], binwidth=0.005)
