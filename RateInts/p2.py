...
for i in range(len(segs)):
    ...
    if not all_completed:
        ...
        print('>>>> --- Additional fast-read background correction...')
        corrected_data_bkg1 = np.ones(corrected_data.shape)
        for integration in tqdm(range(corrected_data.shape[0])):
            corrected_data_bkg1[integration,:,:], _ = reduce.row_by_row_bkg_sub(corrected_data[integration,:,:], mask=m2*mask_bcr[integration,:,:])
        print('>>>> --- Done!!')
